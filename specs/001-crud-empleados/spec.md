...
1. **Given** un usuario autenticado y sin registros previos, **When** registra un empleado con campos válidos, **Then** el sistema genera una `clave` con formato `EMP-` + consecutivo numérico, confirma el alta y guarda el registro.
...
- Intento de crear empleado forzando manualmente un valor de `clave`.
...
- Consulta, actualización o eliminación con `clave` que no cumple el patrón `EMP-[número]`.
...
- **FR-002**: `clave` MUST ser generada automáticamente por el sistema y actuar como identificador primario compuesto lógico del empleado (`prefijo fijo` + `consecutivo numérico`).
...
- **FR-013**: La `clave` generada MUST iniciar siempre con el prefijo `EMP-` seguido de un valor numérico autoincremental sin duplicados.
- **FR-014**: El sistema MUST ignorar o rechazar cualquier intento de asignar `clave` desde la solicitud de creación.
...
- **Empleado**: Representa una persona registrada en el sistema con `clave` generada automáticamente con formato `EMP-<autonumérico>`, además de `nombre`, `dirección` y `teléfono`.

## Assumptions

- `clave` no es proporcionada por el cliente; se genera automáticamente en backend.
- El componente numérico de `clave` es secuencial y no se reutiliza, incluso si se elimina un empleado.
...
- **SC-003**: El 100% de altas generan una `clave` válida con patrón `EMP-` + número autoincremental y sin duplicados.
...
- **SC-006**: El 100% de intentos de establecer manualmente `clave` en creación son rechazados o ignorados según el contrato definido.
